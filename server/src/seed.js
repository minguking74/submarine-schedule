...
function seed() {
  ...
  const alreadySeeded = db.prepare(`SELECT COUNT(*) as c FROM capacity_design`).get().c > 0;
  ...
  const insertCapacityDesign = db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?)
  `);
  raw.capacity_design.rows.forEach((r, i) => {
    const [segType, segmentId, segmentLabel, design] = r;
    insertCapacityDesign.run(segType, segmentId, segmentLabel, num(design), i);
  });
  ...
  const insertPhase = db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `);
  raw.capacity_phase.rows.forEach((r, i) => {
    const [label, cur, p1, p2, p3, p4] = r;
    if (!label || label === '합계') return;
    insertPhase.run(label, num(cur), num(p1), num(p2), num(p3), num(p4), i);
  });
  ...
  const insertLightup = db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
  raw.lightup_schedule.rows.forEach((r) => {
    const [schedule, segType, segmentId, if100g, if400g, , unitPrice, , status, completeDate, note] = r;
    insertLightup.run(schedule, segType, segmentId, num(if100g), num(if400g), num(unitPrice), status || '계획중', completeDate || null, note || null);
  });
  ...
  raw.funnel.rows.forEach((r) => {
    ...
    insertFunnel.run(num(no), customer, aEnd, zEnd, num(if100g), num(if400g), stage || 'Prospect', priority || 'Medium', toDateOnly(targetDate), note || null);
  });
  ...
  const insertDemandS = db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
  (raw.internal_demand_s?.rows || []).forEach((r, i) => {
    const [dls, label, h1, h2, h3, h4, h5, h6] = r;
    insertDemandS.run(dls || null, label || null, num(h1), num(h2), num(h3), num(h4), num(h5), num(h6), i);
  });
  ...
  const insertDemandL = db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `);
  (raw.internal_demand_l?.rows || []).forEach((r, i) => {
    const [pop, label, h1, h2, h3, h4, h5] = r;
    insertDemandL.run(pop || null, label || null, num(h1), num(h2), num(h3), num(h4), num(h5), i);
  });

  const insertKpi = db.prepare(`INSERT INTO kpis (year, memo) VALUES (?, ?)`);
  raw.kpis.rows.forEach((r) => {
    const [year, memo] = r;
    if (year) insertKpi.run(Number(year), memo || null);
  });

  setFxRate(1380);

  // Log sheet rows are [No, Dates, 내용]
  raw.log.rows.forEach((r) => {
    const [, , , dateStr, content] = r;
    if (content) {
      const message = String(content).trim();
      const at = toDateOnly(dateStr) || undefined;
      if (at) {
        db.prepare(`INSERT INTO activity_log (at, message) VALUES (?, ?)`).run(at, message);
      } else {
        logActivity(message);
      }
    }
  ...
  logActivity('초기 데이터 임포트 (SJC2_Capacity_Schedule_v2_3_3.xlsx)');
